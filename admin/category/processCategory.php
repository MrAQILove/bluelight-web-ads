...
$catParentId = (int)$_GET['catParentId'];
...
switch ($action) {
	...
	case 'activate' :
        activateCampaign($catParentId);
        break;

	case 'deactivate' :
        deactivateCampaign($catParentId);
        break;
    ...
    default :
        ...
        header('Location: /ajaxLogin/index.php');
}
...
function addCategory()
{
    ...
    $sql   = "INSERT INTO tbl_category (cat_parent_id, cat_name, cat_description, cat_image, cat_year_created, cat_active) 
              VALUES ($parentId, '$name', '$description', '$catImage', '$current_year', '$active')";
...
function modifyCategory()
{
    ...
    $sql    = "UPDATE tbl_category 
               SET cat_name = '$name', cat_description = '$description', cat_image = $catImage, cat_active = '$active'
               WHERE cat_id = $catId";
...
/*
    Activate a Campaign
*/
function activateCampaign(&$catParentId)
{
    $catID	= (int)$_GET['catId'];
    ...
    $result = dbQuery($sql) or die('Cannot activate Campaign. ' . mysql_error());
    header('Location: index.php?catId='.$catParentId);              
}

/*
    Deactivate a Campaign
*/
function deactivateCampaign(&$catParentId)
{
    $catID	= (int)$_GET['catId'];
	...
	$result = dbQuery($sql) or die('Cannot deactivate Campaign. ' . mysql_error());
    header('Location: index.php?catId='.$catParentId);              
}
